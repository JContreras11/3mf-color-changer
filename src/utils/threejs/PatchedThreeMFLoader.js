...
class ThreeMFLoader extends Loader {
	...
	parse( data ) {
		...
		function getModelReferencePath( node ) {
		...
		function parseComponentNode( componentNode ) {
			...
			const path = getModelReferencePath( componentNode );
		...
		function parseBuildNode( buildNode ) {
			...
			for ( let i = 0; i < itemNodes.length; i ++ ) {
				...
				const path = getModelReferencePath( itemNode );
		...
		function resolveModelPath( path, currentModelPath ) {

			if ( ! path ) return currentModelPath;
			...
			const baseParts = normalizeModelPath( currentModelPath ).split( '/' );
			baseParts.pop();

			const resolved = [];
			const parts = baseParts.concat( normalizeModelPath( path ).split( '/' ) );
		...
		function buildComposite( compositeData, objects, modelData, textureData, objectData ) {
			...
			for ( let j = 0; j < compositeData.length; j ++ ) {
				...
				const componentModelPath = resolveModelPath( component.path, objectData.modelPath );
				const componentModelData = objectData.modelsData[ componentModelPath ];

				if ( componentModelData === undefined ) {

					throw new Error( `THREE.ThreeMFLoader: Cannot resolve component model path ${ componentModelPath }.` );

				}

				let build = objects[ getObjectKey( componentModelPath, component.objectId ) ];

				if ( build === undefined ) {

					buildObject( component.objectId, objects, componentModelData, textureData, componentModelPath, objectData.modelsData );
					build = objects[ getObjectKey( componentModelPath, component.objectId ) ];
		...
		function buildObject( objectId, objects, modelData, textureData, modelPath, modelsData ) {

			const objectData = modelData[ 'resources' ][ 'object' ][ objectId ];

			if ( objectData === undefined ) {

				throw new Error( `THREE.ThreeMFLoader: Cannot resolve object ${ objectId } in model ${ modelPath }.` );

			}

			objectData.modelPath = modelPath;
			objectData.modelsData = modelsData;
			...
			if ( objectData[ 'mesh' ] ) {
				...
				objects[ getObjectKey( modelPath, objectData.id ) ] = getBuild( meshData, objects, modelData, textureData, objectData, buildGroup );

			} else {

				const compositeData = objectData[ 'components' ];

				objects[ getObjectKey( modelPath, objectData.id ) ] = getBuild( compositeData, objects, modelData, textureData, objectData, buildComposite );

			}

			if ( objectData.name ) {

				objects[ getObjectKey( modelPath, objectData.id ) ].name = objectData.name;
		...
		function build( objects, data3mf ) {
			...
			for ( let i = 0; i < buildData.length; i ++ ) {
				...
				const buildModelPath = resolveModelPath( buildItem.path, modelPath );
				const object3D = objects[ getObjectKey( buildModelPath, buildItem[ 'objectId' ] ) ].clone();
